...
Complete REST API endpoint documentation for ArgoCD (v3.0+).

**Important**: This reference documents the latest stable ArgoCD API. API endpoints and response formats may vary in older versions. Always verify compatibility with your deployed ArgoCD version.

## Version Compatibility

- **Documented for**: Latest stable ArgoCD (v3.0+)
- **Minimum version**: ArgoCD v3.0
- **Query current version**: `GET /api/version`

To check your ArgoCD API version:

```bash
curl -s -H "Authorization: Bearer $ARGOCD_AUTH_TOKEN" \
  "https://$ARGOCD_SERVER/api/version" | jq '.Version'
```
