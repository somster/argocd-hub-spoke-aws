...
**CRITICAL**: All recommendations MUST use the latest stable versions at invocation time. Before generating code:

1. **Terraform**: Fetch latest stable version
   ```bash
   # Get latest stable Terraform
   curl -s https://api.github.com/repos/hashicorp/terraform/releases | \
     jq -r '.[] | select(.prerelease==false) | .tag_name' | head -1
   ```
   Use `required_version = ">= X.Y.Z"` to ensure latest stable is used.

2. **AWS Provider**: Fetch latest stable AWS provider
   ```bash
   # Get latest stable AWS provider
...
3. **ArgoCD**: Fetch latest stable ArgoCD version
   ```bash
   # Get latest stable ArgoCD
   curl -s https://api.github.com/repos/argoproj/argo-cd/releases | \
     jq -r '.[] | select(.prerelease==false) | .tag_name' | head -1
   ```

4. **Docker/Kubernetes**: Always reference latest stable base images and API versions

Update version constraints dynamically—never hardcode versions from the past. If version fetch fails, use the minimum known stable version and warn the user.
...
- **Use latest stable versions only (fetch at invocation time)**
...
- **Use hardcoded or outdated versions**
- **Use pre-release or alpha/beta versions in production code**
